class Report < ActiveRecord::Base
  ...
  def last_meter
    Meter.includes(:report).where(["reports.car_id = ? AND reports.started_at < ? AND meters.deleted_at IS NULL", self.car_id, self.started_at]).last || Meter.new(:meter => 0, :mileage => 0, :riding_mileage => 0, :riding_count => 0, :meter_fare_count => 0)
  end
end
