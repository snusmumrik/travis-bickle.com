...
(->
  ...
  $("#debit").html(String(parseInt($("#report_cash").val()) + parseInt($("#report_edy").val()) + parseInt($("#report_ticket").val()) + parseInt($("#report_advance").val()) + parseInt($("#report_fuel_cost").val()) + parseInt($("#report_account_receivable").val()) + parseInt($("#report_deficiency_account").val()) + parseInt($("#debit_amount").val())).replace( /(\d)(?=(\d\d\d)+(?!\d))/g, '$1,'))
)()
...
$("input[id^='report_']").change( ->
  $("#credit").html(String(parseInt($("#report_sales").val()) + parseInt($("#report_extra_sales").val()) + parseInt($("#report_surplus_funds").val())).replace( /(\d)(?=(\d\d\d)+(?!\d))/g, '$1,'))
  $("#report_cash").val($("#credit").html().replace(",", "") - $("#report_edy").val() - $("#report_ticket").val() - $("#report_advance").val() - $("#report_fuel_cost").val() - $("#report_account_receivable").val() - $("#report_deficiency_account").val() - $("#debit_amount").val())
